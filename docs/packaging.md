...
tar -xzf myhelp-cli-x86_64-unknown-linux-gnu-v0.9.0.tgz
mkdir -p "$HOME/.local/bin"
install -m 0755 \
  myhelp-cli-x86_64-unknown-linux-gnu-v0.9.0/myhelp \
...
```bash
archive=myhelp-cli-aarch64-apple-darwin-v0.9.0.tgz
grep -F "  $archive" SHA256SUMS | shasum -a 256 --check
tar -xzf "$archive"
mkdir -p "$HOME/.local/bin"
install -m 0755 \
  myhelp-cli-aarch64-apple-darwin-v0.9.0/myhelp \
  "$HOME/.local/bin/myhelp"
myhelp --version
```
...
```powershell
$archive = "myhelp-cli-x86_64-pc-windows-msvc-v0.9.0.tgz"
$expected = (
...
Copy-Item `
  "myhelp-cli-x86_64-pc-windows-msvc-v0.9.0\myhelp.exe" `
  "$HOME\bin\myhelp.exe"
...
gh attestation verify \
  myhelp-cli-x86_64-unknown-linux-gnu-v0.9.0.tgz \
  --repo hjosugi/myhelp
```
...
```bash
nix run github:hjosugi/myhelp/v0.9.0 -- --version
nix run github:hjosugi/myhelp/v0.9.0 -- list
```
...
node scripts/release.mjs smoke-cli \
  release-assets/myhelp-cli-x86_64-unknown-linux-gnu-v0.9.0.tgz \
  x86_64-unknown-linux-gnu
```
