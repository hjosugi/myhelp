...
| Accidental publication or history loss through Git sync | Per-vault opt-in in local Git config; new pages need `--include-new`; conflict copies and recovery files go to `.git/info/exclude`; no force-push, rebase, reset, or automatic conflict resolution; no credential handling (`GIT_TERMINAL_PROMPT=0`) | Pull and push act on the whole repository when the vault is a subdirectory; the user's own Git hooks run |
